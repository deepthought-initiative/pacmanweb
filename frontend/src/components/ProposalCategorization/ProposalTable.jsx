...
const ProposalTable = ({ handleClick, currentId, setShowTable, setShowLogs }) => {
    ...
    useEffect(() => {
        async function fetchTable() {
            ...
            const tableResponse = await fetch(`http://127.0.0.1:5000/api/outputs/proposal_cat_output/${currentId}`, {
                method: "GET",
    ...
    return (
        <>
                ...
                <div className="col-md-9">
                    ...
                    <div className='table-container'>
                        <table className='container-fluid'>
                            ...
                            <tbody>
                                {dataToDisplay && dataToDisplay["proposal_table"] && Object.entries(dataToDisplay["proposal_table"]).map(([key, value]) => (
                                    <tr onClick={() => handleHighlight(key)} className={highlighted === key ? "highlighted" : ""} key={key}>
                                        <td className="col-2 text-break" scope="row">{key}</td> {/*Proposal Number*/}
                                        <td className="col-4 text-break">{/*title*/}</td>
                                        <td className="col-6 text-break">{value["PACMan Science Category"]}</td>
                                        <td className="col-2 text-break">{value["PACMan Probability"]}</td>
                                        <td className="col-4 text-break">{value["Original Science Category"]}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    ...
                    <div className='table-container rounded-4'>
                        <table className="container-fluid">
                            ...
                            <tbody>
                                {data.map((row) => (
                                    <tr key={row.id}>
                                        <td className="col-6 text-break">{row.column2}</td>
                                        <td className='col-2 text-break'>{row.column3}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
            ...
            <div className='button-tray container-fluid p-0'>
                <button className='btn rounded-0' onClick={anotherCycle}>Categorize Another Cycle</button>
                <a href={encodedUri} download='proposals.csv'>
                    <button className='btn rounded-0'>Download As CSV</button>
                </a>
                <button className='btn rounded-0' onClick={viewLogs}>View Logs</button>
            </div>
        </>
    )
}
