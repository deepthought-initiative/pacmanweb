...
from pacmanweb import Config, create_app
...
@pytest.fixture()
def auth_header():
    credentials = b64encode(f"default:{Config.DEFAULT_PASSWORD}".encode()).decode("utf-8")
...
class TestAPI:
    ...
    def test_prev_result(self, client, auth_header):
        for task_hash in self.task_hashes:
            while True:
                response = client.get(
                    f"/api/prev_runs/{task_hash}", headers=auth_header
                )
